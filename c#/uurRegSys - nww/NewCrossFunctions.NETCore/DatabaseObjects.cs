...
namespace NewCrossFunctions.NETCore {
    public class DatabaseObjects {
        ...
        private static DateTime ReadFromReader(IDataRecord _record, string _readProp) {
            try {
                if (_record.IsDBNull(_record.GetOrdinal(_readProp))) { return new DateTime(); }

                string bamnanan = _record.GetValue(_record.GetOrdinal(_readProp)).ToString();
                return DateTime.Parse(bamnanan);

                //return _record.GetDateTime(_record.GetOrdinal(_readProp));

            }
            catch(Exception ex) {
                
                string balalal = ex.Message;
        ...
        */
        ...
        public List<UserTableTableEntry> GetListUTFromReader(SqlDataReader _reader) // als er niets is pak hij alsnog een 
        {
            ...
            while (_reader.Read()) {
                ...
                if (fields.Contains(UserTableNames.DateJoined)) { entry.DateJoined = ReadFromReader((IDataRecord)_reader, UserTableNames.DateJoined); }
                if (fields.Contains(UserTableNames.DateLeft)) { entry.DateLeft = ReadFromReader((IDataRecord)_reader, UserTableNames.DateLeft); }
        ...
        public List<RegistratieTableTableEntry> GetListRTFromReader(SqlDataReader _reader) {
            ...
            while (_reader.Read()) {
                ...
                if (fields.Contains(RegistratieTableNames.Date)) {
                    entry.Date = ReadFromReader((IDataRecord)_reader, RegistratieTableNames.Date);
                }

                if (fields.Contains(RegistratieTableNames.TimeInteken)) {
                    entry.TimeInteken = ReadFromReader((IDataRecord)_reader, RegistratieTableNames.TimeInteken).TimeOfDay;
                }

                if (fields.Contains(RegistratieTableNames.TimeUitteken)) {
                    entry.TimeUitteken = ReadFromReader((IDataRecord)_reader, RegistratieTableNames.TimeUitteken).TimeOfDay;
                }
